...
// console.log("event.remaining:", event?.remaining, "event.totalTickets:", event?.totalTickets);
...
//     // ipfsCID is now defined because you added it to props
//     if (!ipfsCID) {
//       throw new Error('Missing IPFS CID for this event metadata.');
//     }

//     // Call purchaseTicket with both CIDs
...
//   return (
//     <div className={`flex flex-col overflow-hidden rounded-3xl ${styles.blur} ${styles.cardBg} ${styles.border} ${styles.shadow} ${styles.shadowHover} transition-shadow duration-300`}>
...
//         />
//         <div className={`absolute top-4 right-4 px-3 py-1 rounded-full ${styles.blur} ${styles.accentText} ${styles.inputBg} ${styles.inputBorder} ${styles.buttonText} text-sm font-semibold shadow`}>
...
//         <div className="mb-4">
//           <h3 className={`truncate text-xl font-semibold ${styles.primaryText}`}>{name}</h3>
//           <p className={`mt-1 text-sm ${styles.secondaryText}`}>{formatDate(date)}</p>
//           <p className={`mt-2 text-sm ${styles.secondaryText}`}>🎟️ Tickets Remaining: {remainingCount} / {totalCount}</p>
//         </div>
...
//               loading || maxQty === 0 ? styles.buttonDisabled : `${styles.buttonBg} ${styles.buttonText}`
//             }`}
//           >
//             {loading
//               ? 'Processing…'
